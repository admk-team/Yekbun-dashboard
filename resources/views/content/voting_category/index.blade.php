...
<div class="container">
    ...
    <div class="d-flex justify-content-center mt-2 mb-2">
        <button class="btn btn-primary col-md-3" data-bs-toggle="modal" data-bs-target="#newscategorymodel">Add Category</button>
    </div>
</div>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <form method="POST" action="{{ route('vote-category.store') }}">
                        @csrf
                        <div class="col mb-3">
                            <label for="nameLarge" class="form-label">Name</label>
                            <input type="text" id="nameLarge" class="form-control" placeholder="Enter Name" name="vote_category">
                        </div>
                        ...
                        <div class="dropdown d-inline-block show">
                            ...
                            <div tabindex="-1" role="menu" aria-hidden="true" class="dropdown-menu-xl dropdown-menu" x-placement="top-start" style="position: absolute; transform: translate3d(0px, -362px, 0px); top: 0px; left: 0px; will-change: transform;min-width: 9rem;">
                                <ul class="nav flex-column">
                                    <li class="nav-item">
                                        <a href="{{ route('votecat-status',['id'=>$vote->id,'status'=>1]) }}" class="nav-link">Active
                                        </a>
                                    </li>
                                    <li class="nav-item">
                                        <a href="{{ route('votecat-status',['id'=>$vote->id,'status'=>0]) }}" class="nav-link">Deactive</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="modal fade" id="editvotingcategoryModal{{ $vote->id }}" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-lg" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="row">
                                            <form method="POST" action="{{ route('vote-category.update',$vote->id) }}">
                                                @csrf
                                                @method('put')
                                                <div class="col mb-3">
                                                    <label for="nameLarge" class="form-label">Name</label>
                                                    <input type="text" id="nameLarge" class="form-control" placeholder="Enter Name" name="vote_category" value={{ $vote->name ?? '' }}>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-label-secondary" data-bs-dismiss="modal">Close</button>
                                                    <button type="submit" class="btn btn-primary">Save changes</button>
